...
import { GemCardComponent } from '../gem-card/gem-card.component';
import { GemDetailsComponent } from '../gem-details/gem-details.component';

@Component({
  selector: 'app-gem-list',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule, GemCardComponent, GemDetailsComponent],
...
export class GemListComponent implements OnInit, OnDestroy {
  ...
  onGemUpdated(updatedGem: Gem): void {
    // Update gem in the list
    const list = this.gems().map(g => g.id === updatedGem.id ? updatedGem : g);
    this.gems.set(list);
    // Update selected gem
    this.selectedGem.set(updatedGem);
  }

  onDetailsError(errorMessage: string): void {
